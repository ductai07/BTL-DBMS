...
const Promotions = () => {
  ...
  const fetchPromotions = async () => {
    ...
    try {
      ...
      if (defaultType !== promotionTypes[0].value) {
        const typeValue =
          defaultType === "Fixed Amount" ? "fixed" : "percentage";
        url += `&type=${typeValue}`;
      }
      ...
      const formattedPromotions = responseData.data.map((discount) => ({
        ...
        type:
          discount.discountType === "fixed" || discount.type === "fixed"
            ? "Fixed Amount"
            : "Percentage",
        ...
        status:
          discount.status ||
          determineStatus(
            discount.startDate || discount.startTime,
            discount.endDate || discount.endTime
          ),
  ...
  const [defaultStatus, setDefaultStatus] = useState(() => {
    return (
      localStorage.getItem("keyPromotionStatus") || promotionStatuses[0].value
    );
  });
  ...
  const [defaultScope, setDefaultScope] = useState(() => {
    return (
      localStorage.getItem("keyPromotionScope") || applicationScope[0].value
    );
  });
  ...
  const expiredPromotions = data.filter(
